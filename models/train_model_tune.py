...
def get_search_space():
    return {
        'percentage': tune.choice([100]),
        'n_piezo_connected': tune.choice([3,4, 5, 6]),
        'W': tune.choice([5, 10, 15]),
        'gcn_true': tune.choice([True]),
        'build_adj': tune.choice([False]),
        ...
        'graph_type' : tune.choice(['geolayer','rf', 'rf_feature_importance' 'rf_feature_importance_no_exo',  'regis_layer']),
        'feature_importance_multiplier' : [0.1, 1, 5]
    }
...
def main():
    ...
    A_tilde, static_features = gnn_data_prep.main(df_piezo_columns, pump_columns, locations_no_missing, config['graph_type'], config['percentage'] , config['n_piezo_connected'], config['feature_importance_multiplier'] )
    ...
    layer_info = (pd
                  .read_csv(PIEZO_LAYER_INFORMATION)
    ...
    geolayer_summary = (
        merged
        .groupby('geolayer')['rmse']
        .agg(['mean','std'])
        .reset_index()
        .rename(columns={'mean':'RMSE Mean','std':'RMSE StdDev'})
    )

    # 3) Build the row dict
    row = {
        "Timestamp": datetime.datetime.now().strftime("%Y%m%d_%H%M%S"),
        "Graph Type":             config["graph_type"],
        "Percentage":             config["percentage"],
        "Piezometer Connections": config["n_piezo_connected"],
        "Overall RMSE Mean":      float(np.mean(test_rmse)),
        "Overall RMSE StdDev":    float(np.std(test_rmse)),
    }
    ...
    ts = row["Timestamp"]
    df_summary.to_csv(
        TRAINING_SUMMARIES / f"summary_{ts}.csv",
        index=False
    )
    model_type = 'MTGNN'
...
def train_tune(config):
    ...
    train_data, val_data, test_data, train_mask, val_mask, test_mask, \
      df_piezo_columns, pump_columns, locations_no_missing, scaler = \
        process_data.main(config['synthetic_data'])

    A_tilde, static_features = gnn_data_prep.main(
        df_piezo_columns,
        pump_columns,
        locations_no_missing,
        config['graph_type'],
        config['percentage'],
        config['n_piezo_connected'],
        config['feature_importance_multiplier']
    )
    ...
    # 3) Train (reuse your existing train() but strip out file‐i/o)
    train(
        ...
        A_tilde=A_tilde, F_w=F_w, W=W,
        config=config
    )

    # 4) Evaluate on test split
    test_sample = AutoregressiveTimeSeriesDataset(
        test_data, input_window=W, max_future_window=F_w,
        missing_data_mask=test_mask, num_piezo=num_piezo
    )[1]
    test_input, test_predicted, test_target = make_predictions(
        model, test_sample, device, F_w, W, A_tilde, static_features, num_piezo
    )

    # inverse‐scale
    pred_ = inverse_transform_with_shape_adjustment(test_predicted.numpy(), scaler, num_piezo)
    targ_ = inverse_transform_with_shape_adjustment(test_target.numpy(), scaler, num_piezo)

    # RMSE per node
    test_rmse = calculate_rmse_per_piezometer(pred_, targ_, num_piezo)
    mean_rmse = float(np.mean(test_rmse))
    std_rmse  = float(np.std(test_rmse))

    # 5) Report back to Tune
    tune.report(
        eval_loss=mean_rmse,
        eval_std=std_rmse
    )
...
scheduler = ASHAScheduler(metric="eval_loss", mode="min", grace_period=10)
reporter  = CLIReporter(
    parameter_columns=["graph_type","percentage","n_piezo_connected","W"],
    metric_columns=["eval_loss","eval_std","training_iteration"]
)

analysis = tune.run(
    train_tune,                # your function
    config=get_search_space(), # your search space
    num_samples=20,
    scheduler=scheduler,
    progress_reporter=reporter,
    resources_per_trial={"cpu":2,"gpu":1},
    storage_path=TRAINING_SUMMARIES / "ray_results", 
    name="mtgnn_hpo"
)

print("Best config:", analysis.get_best_config("eval_loss","min"))