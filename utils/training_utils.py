...
def generate_model_filename(model_type, future_window, graph_type=None, **kwargs):
    """
    Generate a model filename that includes model type, future window, and optionally graph type and key config values.
    """
    base_name = f"{model_type}_fw{future_window}"

    if graph_type:
        base_name += f"_{graph_type}"

    if 'W' in kwargs:
        base_name += f"_W{kwargs['W']}"

    if 'n_piezo_connected' in kwargs:
        base_name += f"_N_Piezo:_{kwargs['n_piezo_connected']}"

    if 'feature_importance_multiplier' in kwargs:
        base_name += f"_FIM:_{kwargs['feature_importance_multiplier']}"

    if 'weight_mode' in kwargs:
        base_name += f"_WM:_{kwargs['weight_mode']}"
