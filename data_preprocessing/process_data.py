...
from config import (
    PIEZO_METADATA, PUMP_METADATA, EVAP_METADATA,
    PREC_METADATA, RIVER_METADATA,
    PREPROCESSED_DIR, PROCESSED_DATA_FILE,
    COLUMN_NAMES_REAL, COLUMN_NAMES_SYN,
    PUMPING_WELLS_PATH, PRECIP_PATH,
    EVAP_PATH, RIVER_PATH, INPUT_DIR
)
...
def process_synthetic_data(synthetic_data_path, real_data_path, series_names, config):
    ...
    if additional_needed > 0:
        ...
        piezo_metadata = pd.read_csv(PIEZO_METADATA)
...
def load_external_data(data_path, common_start_date, common_end_date, resampling_freq):
    ...
    df_pumping_wells = pd.read_csv(PUMPING_WELLS_PATH, index_col='Datum', parse_dates=True)
    ...
    df_precipitation = pd.read_csv(PRECIP_PATH)
...
def main(synthetic_data=False, resampling_freq='W', val_split=None, test_val_size=0.2):
    ...
    # Forward-fill then back-fill small gaps so daily resampling doesn't
    # drop river stations that are complete at weekly resolution.
